...
function upload_file($field_name) {
	...
	if(isset($_FILES[$field_name])) {
		...
		$file_name = sanitize_file_name($_FILES[$field_name]['name']); //Name is given by php
		$file_extension = file_extension($file_name);

		// Even more secure to assign a new name of your choosing.
		//Vu note: generates a unique name without your input.
		// Example: 'file_536d88d9021cb.png'
		$unique_id = uniqid('file_', true);						//Uniqueid is php function
		$new_name = "{$unique_id}.{$file_extension}";

		$_FILES[$field_name]['name'] = $new_name;
